# 改动记录:拆分为本地 SwiftPM 包 + Xcode App 工程

- **日期**:2026-06-23
- **更新**:2026-06-28,`LotteryApp/` Xcode 工程已提交;临时 `LotteryChecker-Sources/` 已移除。
...
LotteryApp/                       # 已创建好的 macOS Xcode App 工程
├── LotteryApp.xcodeproj          # 引用 ../LotteryKit 本地包
├── LotteryApp/                   # App 的 SwiftUI 源码 + Assets.xcassets
├── LotteryAppTests/
└── LotteryAppUITests/

XCODE-SETUP.md                    # 工程说明 + 配沙盒上架的步骤
```

要点:

- 2026-06-23 先把 SwiftUI 源码暂存到 `LotteryChecker-Sources/`,用于手动创建 Xcode 工程。
- 2026-06-28 Xcode 工程创建完成后,SwiftUI 源码已迁入 `LotteryApp/LotteryApp/`,旧暂存目录删除。
- 所有源码用 `git mv`/迁移方式保留历史。
...
2. 打开 [`../../LotteryApp/LotteryApp.xcodeproj`](../../LotteryApp/LotteryApp.xcodeproj),确认本地包 `../LotteryKit` 已接入 → 配 Sandbox/签名/图标 → Archive 上架。
...
- ⚠️ 引用旧 `macapp/` 或 `LotteryChecker-Sources/` 路径的脚本/文档需更新(README 与 Xcode 文档已同步)。
